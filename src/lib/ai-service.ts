...
export class AIService {
  ...
  /**
   * Generates a companion response based on the primary model (Llama 3.1 8B)
   * with automatic failover to the secondary model (Gemini 2.5 Flash) on failure/timeout.
   */
  async generateCompanionResponse(
    ...
    // Parse health summary and memory profile from healthSummary parameter
    ...
    // Check if greeting or simple conversational chat
    ...
    // Check if report analysis or direct wellness request
...
- Profile: ${memoryObj ? JSON.stringify(memoryObj) : 'No memory profile'}`;
    ...
    if (this.groq) {
      try {
        ...
        // Call with a 6-second timeout fallback trigger
      ...
      // If Groq key is not defined, run Gemini directly
      if (this.gemini) {
        try {
          const responseText = await this.queryGemini(systemPrompt, history, message);
          return { response: responseText, modelUsed: 'gemini-2.5-flash' };
    ...
    return { response: 'AI API keys not configured. Please set GROQ_API_KEY or GEMINI_API_KEY in .env.local', modelUsed: 'llama-3.1-8b' };
  }

  /**
   * Helper to query Gemini 2.5 Flash
   */
  private async queryGemini(systemInstruction: string, history: ChatMessage[], message: string): Promise<string> {
    ...
    // Google Generative AI model selection
    // Support gemini-2.5-flash, fallback to gemini-1.5-flash if needed
    const model = this.gemini.getGenerativeModel({
      model: 'gemini-2.5-flash',
      systemInstruction: systemInstruction,
    });

    // Map history to contents structure required by Google Generative AI
    const contents = [
      ...history.map(m => ({

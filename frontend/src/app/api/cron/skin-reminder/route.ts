...
import { getCronSupabaseClient, fetchWeatherForCron, buildSkinMessage, validateCronRequest } from '@/lib/services/cron-utils';
...
export async function GET(req: Request) {
  try {
    ...
    // Check skin_logs for today
    const { data: skinLogs } = await supabase
      .from('skin_logs').select('user_id').in('user_id', userIds).eq('date', todayStr);
    const skinDone = new Set((skinLogs || []).map((s: any) => s.user_id));
    ...
    for (const userId of userIds) {
      if (skinDone.has(userId)) continue;
      ...
      for (const sub of (userSubMap.get(userId) || [])) {
        const r = await sendWebPush({ endpoint: sub.endpoint, keys: { p256dh: sub.p256dh, auth: sub.auth } },
          { title, message: body, url: '/skin', actionLabel: 'Log Skin Today', tag: 'skin-reminder', category: 'skin' });
